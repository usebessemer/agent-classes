...
| [`skills/`](skills/) | One module per charter skill. Slice 1: [`ingest_and_align`](skills/ingest_and_align.py). Slice 2: [`flag_variance`](skills/flag_variance.py). Slice 3: [`build_report`](skills/build_report.py). Slice 4: [`explain_variance`](skills/explain_variance.py). |

## Status

Charter v0.1 · slices 1–4 built.
...
- **`explain_variance`** — **built** (slice 4). Reads a composed `ReportPackage` and strikes the story `build_report` deferred: per target it **differences** the actuals-to-date against the summed budget referents into one signed `target_variance`, classifies it over-/under-budget (`None` at exactly zero), **ranks** the target's flagged variances into a full ordered list of `VarianceDriver`s (largest `abs(delta)` first, each field carried **verbatim** off its flag), and discloses the `subfloor_remainder` those drivers do not cover. **Option A, the deterministic rung** — it strikes and ranks the variance that *already exists*: it authors **no** generated prose (the LLM-narrative onset is deferred to a later, more robust analyst class) and forecasts **nothing** (§2). A pure, **sync** striker that drives no port and writes nothing; `PROPOSED`-only, mutating neither input. Its explanation model (`VarianceDriver` / `TargetExplanation` / `ExplainedPackage`) plus the `explain_variance` operation dual-export from `jr_analyst` and `jr_analyst.skills` — all four names, each the same object across both paths.
- **Input extensions** — **planned**. The forward-looking `committed` (an open PO) / `anticipated` (a not-yet-committed cost) ladder rungs are defined on the ladder today so the grade surface is stable, but their line types and their flow into alignment are deferred (per CHARTER §2). The predictive **forecaster** overlay — trend / scenario / run-rate / EAC — stays explicitly out (CHARTER §2).
