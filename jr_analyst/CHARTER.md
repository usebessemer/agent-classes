...
**[BUILT]** = implemented today · **[PLANNED]** = on the class roadmap (slice 4). The autonomy line for each is in §5. Each skill is a read-only computation over what the ports yield — the analyst counterpart to the Bookkeeper's detection-only `reconcileAccount`: a pure aligner over async read ports (count-conserving), plus downstream pure **sync** readers over the already-aligned dataset, and a pure **sync** assembler that composes those already-computed outputs into the review package — all surface-don't-resolve / assemble-don't-act.
...
- **`build_report()` [BUILT]** — compose the periodic FP&A review package (Contract A): carry slice 1's `AlignedDataset` and slice 2's `VarianceReport` **verbatim** (aligned pairs + variances + escalations, same objects in input order — no delta re-struck, `flag_variance` never re-run) into one **PROPOSED** `ReportPackage`, and add a **per-target roll-up** — group the aligned pairs on the *actual's* attribution target (targets sorted), subtotal each target's actuals **per certainty grade** (exact `Decimal`, ladder-ordered, present grades only — a mixed closed+open target yields two subtotals, never one blended figure), and sum the matched budget referents **separately** into `budget_referent_total` on an `ACTUALS_TO_DATE` basis — carried alongside the actuals and **never differenced**, honestly labeled *the referent-sum at grain `align_on`* (not the target's authored budget; under the coarse default a cross-target referent still aligns and trips `budget_referents_cross_target`, drillable via the roll-up's by-reference `pairs`). Assembly-only — the slice-3 (assembly) side of the slice-3-vs-slice-4 line: no narrative, no ranking, no driver attribution, and no remaining/percent-consumed/EAC/run-rate/projected/over-under-verdict figure anywhere (those are the parked slice-4 `explain_variance` rung, §2). A pure, **sync** assembler that drives no port and writes nothing — `PROPOSED`-only, never published/filed/blocked; the one hard stop is a window mismatch between the two inputs (a fail-fast `ValueError`, not a package state). Slice 3.
...
**Input extensions [PLANNED]:** the forward-looking ladder rungs. `committed` (an open PO) and `anticipated` (a not-yet-committed cost) are defined on the ladder today so the grade surface is stable, but their line types (`CommitmentLine` / `AnticipatedLine`) and their flow into alignment are deferred to a later slice (slice 3's assembler carries only the two realized rungs the pipeline yields — it never synthesises a `committed` / `anticipated` subtotal). They extend the analyst's view past the two realized rungs without changing the read-only shape.
